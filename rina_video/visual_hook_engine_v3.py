...
class VisualHookEngineV3:
    ...
    def analyze(self, source: str | Path, fps: float = 2.0) -> dict[str, Any]:
        ...
        for ts,buf in self._frames(source,fps):
            ...
            score=min(100.0,motion*1.35+contrast*0.35)
            samples.append({"timestamp":round(ts,3),"motion":round(motion,2),"contrast":round(contrast,2),"visual_peak":round(score,2)})
            prev=buf
        ...
        return {"version":self.VERSION,"status":"READY","duration":round(duration,3),"fps":fps,
                "samples":samples,"peaks":chosen,"method":"temporal_motion_contrast","evidence_only":True}
